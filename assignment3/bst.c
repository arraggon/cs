 ...
 */
int bst_height_recursive(struct bst_node* node) {
  ...
  // The height of the current node is 1 more than the height of its tallest subtree
  return 1 + (left_height > right_height ? left_height : right_height);
}
 int bst_height(struct bst* bst) {
  return bst_height_recursive(bst->root);
 }
 ...
 */
int bst_path_sum_recursive(struct bst_node* node, int sum) {
  if (node == NULL) {
    return 0;
  }
  sum -= node->key;
  ...
  return bst_path_sum_recursive(node->left, sum) || bst_path_sum_recursive(node->right, sum);
}

int bst_path_sum(struct bst* bst, int sum) {
  return bst_path_sum_recursive(bst->root, sum);
}
 ...
 */
int bst_range_sum_recursive(struct bst_node* node, int lower, int upper) {
    if (node == NULL) {
        return 0;
    }
    int sum = 0;
    if (node->key > lower) {
        sum += bst_range_sum_recursive(node->left, lower, upper);
    }
    if (node->key < upper) {
        sum += bst_range_sum_recursive(node->right, lower, upper);
    }
...
int bst_range_sum(struct bst* bst, int lower, int upper) {
    return bst_range_sum_recursive(bst->root, lower, upper);
}
...
void bst_iterator_free(struct bst_iterator* iter) {
  if (iter != NULL) {
    // Free the stack used by the iterator
    if (iter->stack != NULL) {
      stack_free(iter->stack);
    }
    // Now free the iterator itself
    free(iter);
  }
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
    if (iter == NULL || iter->stack == NULL) {
        // If the iterator or its stack is not properly initialized, treat as if there are no more elements
        return 0;
    }
...
int bst_iterator_next(struct bst_iterator* iter, void** value) {
    if (iter == NULL || iter->stack == NULL || stack_isempty(iter->stack)) {
        // Iterator is either not initialized properly or there are no more elements to iterate.
        if (value) {
            *value = NULL; // Ensure the value pointer is set to NULL to indicate no more values.
        }
        return 0; // Returning 0 or an appropriate "end" value might depend on your BST's key domain.
    }

    // Pop the top node from the stack, which is the next in-order node.
    struct bst_node* node = stack_pop(iter->stack);
    *value = node->value; // Set the value pointer to the value of the current node.

    // Process the right subtree of the popped node by traversing down to its leftmost node.
    ...
    return node->key; // Return the key of the current node.
}
